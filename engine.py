...
import utils
from losses import HuberLoss, HuberLoss_with_vgg
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer,
    ...
    for step, (batch, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():
            ...
            # images_patch = rearrange(images, 'b c (h p1) (w p2) -> b (h w) (p1 p2 c)', p1=16, p2=16)
            # labels = rearrange(images_patch, 'b n (p1 p2 c) -> b n (p1 p2) c', p1=16, p2=16)
            labels = torch.clone(images)
...
def validate(model: torch.nn.Module, data_loader: Iterable, device: torch.device,
             log_writer=None, val_hint_list=[10]):
    model.eval()
    header = 'Validation'
    ...
    with torch.no_grad():
        for step, (batch, _) in tqdm(enumerate(data_loader), desc=header, ncols=100, total=len(data_loader)):
            ...
            for idx, count in enumerate(val_hint_list):
                ...
                with torch.cuda.amp.autocast():
                    # outputs, _ = model(images_lab.clone(), bool_hint.clone())
                    outputs = model(images_lab.clone(), bool_hint.clone())
                    outputs = torch.cat((images_lab[:, 0:1, :, :], outputs), dim=1)
