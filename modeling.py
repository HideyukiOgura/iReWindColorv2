...
class Block1_patchwork(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp1 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        # self.norm3 = norm_layer(dim)
        self.conv_layer = Cnn_block_patchwork(dim=dim, k=3, window_size=window_size)
        self.window_size = window_size
        # self.mlp2 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        # self.norm4 = norm_layer(dim)
    ...
    def forward(self, x_1, x_2, x_3, x_4):
        if self.gamma_1 is None:
            ...
            x_1_old, x_2_old, x_3_old, x_4_old = x_1, x_2, x_3, x_4
            x_1 = rearrange(x_1, 'b (p1 p2) c -> b c p1 p2', p1=self.window_size, p2=self.window_size)
            x_2 = rearrange(x_2, 'b (p1 p2) c -> b c p1 p2', p1=self.window_size, p2=self.window_size)
            x_3 = rearrange(x_3, 'b (p1 p2) c -> b c p1 p2', p1=self.window_size, p2=self.window_size)
            x_4 = rearrange(x_4, 'b (p1 p2) c -> b c p1 p2', p1=self.window_size, p2=self.window_size)
            ...
            x_1 = rearrange(x_1, 'b c p1 p2 -> b (p1 p2) c')
            x_2 = rearrange(x_2, 'b c p1 p2 -> b (p1 p2) c')
            x_3 = rearrange(x_3, 'b c p1 p2 -> b (p1 p2) c')
            x_4 = rearrange(x_4, 'b c p1 p2 -> b (p1 p2) c')
            x_1 = x_1_old + self.drop_path(x_1)
            x_2 = x_2_old + self.drop_path(x_2)
            x_3 = x_3_old + self.drop_path(x_3)
            x_4 = x_4_old + self.drop_path(x_4)
            # x = x + self.drop_path(self.mlp2(self.norm4(x)))
        else:
            x_1 = self.forward_features(x_1)
            x_2 = self.forward_features(x_2)
            ...
            x_r = torch.cat((x_3, x_4), dim=3)
            x = torch.cat((x_l, x_r), dim=2)
            x = self.gamma_3 * self.conv_layer(x)
            x_l, x_r = torch.chunk(x, 2, dim=2)
            x_1, x_2 = torch.chunk(x_l, 2, dim=3)
            x_3, x_4 = torch.chunk(x_r, 2, dim=3)
            x_1 = rearrange(x_1, 'b c p1 p2 -> b (p1 p2) c')
...
class Block1_pw(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp1 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        # self.norm3 = norm_layer(dim)
        self.conv_layer = Cnn_block_pw(dim=dim, k=3, window_size=window_size)
        # self.mlp2 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        # self.norm4 = norm_layer(dim)
        self.window_size = window_size
...
class Cnn_block_patchwork(nn.Module):
    def __init__(self, dim, k, window_size):
        super().__init__()
...
class IReWindColor_v2(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, out_chans=2, embed_dim=512, depth=12,
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth // 3)]  # stochastic depth decay rule
        self.blocks = nn.ModuleList([Block1_patchwork(
            dim=embed_dim // 4, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, 
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,
            init_values=init_values, use_rpb=use_rpb, window_size=img_size // patch_size)
            for i in range(depth // 3)])
        ...
        dpr2 = [x.item() for x in torch.linspace(0, drop_path_rate, 2 * depth // 3)]  # stochastic depth decay rule
        self.blocks2 = nn.ModuleList([Block1_pw(
            dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, 
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr2[i], norm_layer=norm_layer,
            init_values=init_values, use_rpb=use_rpb, window_size=img_size // patch_size)
            for i in range(2 * depth // 3)])
    ...
    def forward_features(self, x_1, x_2, x_3, x_4):
        x_1 = self.patch_embed(x_1)
        x_1 = x_1 + self.pos_embed.type_as(x_1).to(x_1.device).clone().detach()  # (B, 14*14, 768)
        x_2 = self.patch_embed(x_2)
        x_2 = x_2 + self.pos_embed.type_as(x_2).to(x_2.device).clone().detach()  # (B, 14*14, 768)
        x_3 = self.patch_embed(x_3)
        x_3 = x_3 + self.pos_embed.type_as(x_3).to(x_3.device).clone().detach()  # (B, 14*14, 768)
        x_4 = self.patch_embed(x_4)
        x_4 = x_4 + self.pos_embed.type_as(x_4).to(x_4.device).clone().detach()  # (B, 14*14, 768)

        for blk in self.blocks:
            x_1, x_2, x_3, x_4 = blk(x_1, x_2, x_3, x_4)
        x_1 = self.norm(x_1)
        x_1 = rearrange(x_1, 'b (p1 p2) c -> b c p1 p2', p1=self.window_size, p2=self.window_size)
        x_2 = self.norm(x_2)
        x_2 = rearrange(x_2, 'b (p1 p2) c -> b c p1 p2', p1=self.window_size, p2=self.window_size)
    ...
    def forward(self, x, mask):
        ...
        x_1, x_2 = torch.chunk(x_l, 2, dim=3)
        x_3, x_4 = torch.chunk(x_r, 2, dim=3)
        x_1, x_2, x_3, x_4 = self.forward_features(x_1, x_2, x_3, x_4)
        x_l = torch.cat((x_1, x_2), dim=3)
        x_r = torch.cat((x_3, x_4), dim=3)
        x = torch.cat((x_l, x_r), dim=2)
        x = self.merge_patch(x)
...
@register_model
def irewindcolorv2(pretrained=False, **kwargs):
    model = IReWindColor_v2(
        out_chans=2,
        img_size=224,
        patch_size=16,
        in_chans=4,
